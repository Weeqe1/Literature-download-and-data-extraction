...
class MultiModelClient:
    ...
    def _get_client_for(self, provider: str, model_name: Optional[str]=None, api_key_env: Optional[str]=None, base_url: Optional[str]=None):
        api_key = self._get_api_key(api_key_env)
        
        if provider == 'openai':
            return LLMClient(api_key=api_key, model=model_name, base_url=base_url)
            ...
            return LLMClient(api_key=api_key, model=model_name, base_url=base_url or "https://generativelanguage.googleapis.com/v1beta/openai/")
        elif provider == 'deepseek':
            if not api_key:
                raise ValueError("DeepSeek API key not found in config or env")
            return LLMClient(api_key=api_key, model=model_name, base_url=base_url or "https://api.deepseek.com")
        elif provider == 'grok':
            if not api_key:
                raise ValueError("Grok API key not found in config or env")
            return LLMClient(api_key=api_key, model=model_name, base_url=base_url or "https://api.x.ai/v1")
        else:
    ...
    def extract(self, model_id: str, prompt: str, schema: Optional[Dict]=None, images: Optional[list]=None) -> Dict[str, Any]:
        ...
        client = self._get_client_for(provider, model_name=model_name, api_key_env=api_key_env, base_url=base_url)
