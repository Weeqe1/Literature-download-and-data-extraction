...
def extract_text_with_strings(pdf_path: str) -> str:
    """使用strings命令提取PDF文本"""
    try:
...
class PDFQualityAnalyzer:
    ...
    def analyze_pdf(self, pdf_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 使用strings提取文本
            full_text = extract_text_with_strings(pdf_path)
...
def main():
    ...
    for i, pdf_path in enumerate(pdf_files, 1):
        ...
        if i <= 10 or result["rating"] in ["A", "B", "F"]:
            ...
            if result["is_nfp_related"]:
                print(f"  核心关键词: {', '.join(result['core_keyword_matches'][:3])}...")
    ...
    # 保存详细JSON结果
    json_path = os.path.join(output_dir, "pdf_quality_analysis_strings.json")
    with open(json_path, 'w', encoding='utf-8') as f:
        json.dump(results, f, ensure_ascii=False, indent=2)
    print(f"\n详细分析结果已保存到: {json_path}")
    
    # 创建CSV报告
    csv_path = os.path.join(output_dir, "pdf_quality_report_strings.csv")
    with open(csv_path, 'w', encoding='utf-8-sig', newline='') as f:
        writer = csv.writer(f)
        
        # 写入表头
        headers = [
            "文件名", "文件大小(KB)", "文本长度", "有效PDF", "NFP相关",
    ...
    print(f"\n总PDF文件数: {total}")
    print("\n评级分布:")
    for rating in ["A", "B", "C", "D", "F", "INVALID", "NO_TEXT", "ERROR"]:
