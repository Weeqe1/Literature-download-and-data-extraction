...
def check_pdf_valid(pdf_path: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        if 'pdf document' in file_type:
            ...
            return False, "HTML文档（下载失败）"
        elif 'javascript' in file_type:
            return False, "JavaScript文件（下载失败）"
        elif 'ascii text' in file_type or 'unicode text' in file_type:
            return False, "文本文件（下载失败）"
        else:
            return False, f"未知格式: {file_type[:50]}"
...
def extract_text_from_pdf(pdf_path: str) -> Tuple[str, int]:
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout.strip():
            # 统计页数
            page_result = subprocess.run(['pdfinfo', pdf_path], capture_output=True, text=True, timeout=10)
            pages = 0
        ...
        # 如果pdftotext失败，尝试使用strings
        result = subprocess.run(['strings', pdf_path], capture_output=True, text=True, timeout=30)
...
class PDFQualityAnalyzer:
    ...
    def analyze_pdf(self, pdf_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            # 检查文件是否为有效PDF
            is_valid, file_type = check_pdf_valid(pdf_path)
            if not is_valid:
                result["rating"] = "INVALID"
                result["rating_reason"] = f"不是PDF文件: {file_type}"
                return result
...
def main():
    ...
    for pdf_path in pdf_files:
        ...
        if is_valid:
            ...
            invalid_files.append((pdf_path, file_type))
    
    print(f"有效PDF文件: {len(valid_pdfs)}")
    print(f"无效文件: {len(invalid_files)}")
    
    # 统计无效文件类型
    invalid_types = {}
    for _, file_type in invalid_files:
        invalid_types[file_type] = invalid_types.get(file_type, 0) + 1
    
    print("\n无效文件类型统计:")
    for file_type, count in sorted(invalid_types.items(), key=lambda x: -x[1]):
    ...
    # 分析有效PDF
    print(f"\n开始分析 {len(valid_pdfs)} 个有效PDF文件...")
    results = []
    
    for i, pdf_path in enumerate(valid_pdfs, 1):
        if i % 50 == 0:
            print(f"\n[{i}/{len(valid_pdfs)}] 分析进度...")
        
        result = analyzer.analyze_pdf(pdf_path)
        results.append(result)
        
        if i <= 10 or result["rating"] in ["A", "B", "F"]:
            print(f"\n[{i}] {result['file_name']}")
    ...
    # 创建CSV报告
    csv_path = os.path.join(output_dir, "pdf_quality_report.csv")
    ...
    print(f"\n有效PDF文件数: {total}")
    print("\n评级分布:")
    for rating in ["A", "B", "C", "D", "F", "NO_TEXT", "ERROR"]:
    ...
    recommended = [r for r in results if r["rating"] in ["A", "B"]]
    if recommended:
    ...
    # 保存无效文件列表
    invalid_csv_path = os.path.join(output_dir, "invalid_files_report.csv")
    with open(invalid_csv_path, 'w', encoding='utf-8-sig', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(["文件名", "文件大小(KB)", "文件类型", "状态"])
        for pdf_path, file_type in invalid_files:
