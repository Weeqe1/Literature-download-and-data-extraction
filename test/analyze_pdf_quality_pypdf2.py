...
try:
    ...
    print("警告：PyPDF2未安装，将尝试使用其他方法")
    PDF_LIB = None

def extract_text_from_pdf_pypdf2(pdf_path: str) -> Tuple[str, int]:
    """使用PyPDF2提取PDF文本"""
    try:
    ...
    def analyze_pdf(self, pdf_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if result["file_size_kb"] < 1:
                ...
                result["rating_reason"] = "文件太小，可能是空文件或损坏"
                return result
            
            # 使用PyPDF2提取文本
            full_text, page_count = extract_text_from_pdf_pypdf2(pdf_path)
            
            if not full_text:
                result["rating"] = "INVALID"
                result["rating_reason"] = "PDF解析错误或无文本内容"
                return result
